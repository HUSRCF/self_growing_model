...
最新补充：[共享F特征与撤回距离](.agents/notes/shared_writer_and_revision_window.md)。共享计算在完整轨迹上减少约38% CPU用量，预测仅有舍入差异；强制只撤回最近两步在一个种子出现失败，不能把短缓冲视为已验证的保证。

最新补充：[稀疏搜索与采样尺度](.agents/notes/sparse_search_and_scale.md)。匹配采样尺度后，每5步搜索一次可减少约63%的节点展开，但增加大量离线回退；精确F缓存已验证不改变完整预测结果。尚无跨条件全面占优的方案。
