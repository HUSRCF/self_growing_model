...
def sparse_window(task):
    ...
    search=AdaptiveBeam(min_depth=2,max_depth=2,temperature=1,search_interval=1 if mode=='full' else 5,
             ...
             commit_probe=mode in ('probe','probe_cache','probe_fast','probe_read'),
             rule_cache_size=4096 if mode in ('probe_cache','probe_fast','probe_read') else 0,
             fast_probe=mode in ('probe_fast','probe_read'),read_cache_size=4096 if mode=='probe_read' else 0)
...
def main():
    ...
    p.add_argument('--mode',choices=['checked','sparse','probe','probe_cache','probe_fast','probe_read','full'],default='checked');p.add_argument('--workers',type=int,default=4)
