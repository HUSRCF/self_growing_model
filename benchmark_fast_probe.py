...
def main():
    p=argparse.ArgumentParser();p.add_argument('--read-cache',action='store_true');args=p.parse_args()
    reference,candidate=('probe_fast','probe_read') if args.read_cache else ('probe_cache','probe_fast')
    ...
    for repeat in range(2):
        for j,i in enumerate([0,8,16,24]):
            ...
            assert old[4]==new[4]
            for k in old[5]:
                if k not in ('cpu_seconds','read_cache_hits','read_cache_misses'):assert old[5][k]==new[5][k],k
            assert sum(old[5][k] for k in ['read_cache_hits','read_cache_misses'])==sum(new[5][k] for k in ['read_cache_hits','read_cache_misses'])
    ...
    report=dict(rows=rows,reference=reference,candidate=candidate,outputs_and_search_counters_exact=True,cache_cpu_sum=old,
    ...
    filename='read_cache_paired_cpu.json' if args.read_cache else 'fast_probe_paired_cpu.json'
    Path('adaptive_search_results',filename).write_text(json.dumps(report,indent=2))
